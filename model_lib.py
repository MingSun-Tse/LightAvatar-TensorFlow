...
class Student(tf.keras.Model):
  ...
  def __init__(
    ...
    assert self.dataset in ["GNOME", "FLAME"], (
        "Data '%s' is not supported." % data
    )
  ...
  def _apply_rotation_limit(
      self, rotation: tf.Tensor, limit: tf.Tensor
  ) -> tf.Tensor:
  ...
  def _build_mlp(
    ...
    res_mlp = [
        tf.keras.layers.Dense(
            width, activation=activation, input_shape=(dim_in,)
        )
    ]
    for _ in range(n_resblocks):
      res_mlp.append(
        ...
        if self.expression_scheme == "local_feats_v1":
          if training:
            ...
            local_feats = tf.reshape(
                local_feats, [num_rays, self.n_local_feats, self.local_feat_dim]
            )
  ...
  def get_model_outputs(
    ...
    if training:
      output, internal_results = self.call(
          input_tensor, training=True, step=step
      )
